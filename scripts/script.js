...
const statBase = document.querySelector("#base-stats");
...
const fetchApi = async (pkmnName) => {
  //Agragamos nombres de pokemon de mas de una palabra
...
search.addEventListener("change", async (event) => {
  ...
  //Validacion de los pokemon que no existen
  if (!pkmnData) {
    alert("El pokémon ingresado no existe.");
    return;
  }

  // Se cambia el color de la card dependiendo de pokemon
  const mainColor = typeColors[pkmnData.types[0].type.name];
  statBase.style.color = `rgb(${mainColor[0]}, ${mainColor[1]}, ${mainColor[2]})`;
  pokedex.style.backgroundColor = `rgb(${mainColor[0]}, ${mainColor[1]}, ${mainColor[2]})`;


  // Para la depuración: se eliminará más adelante
  console.log(pkmnData);

  //establece pokemon # en la parte superior de la página

  number.innerHTML = "#" + pkmnData.id.toString().padStart(3, "0");

  // Establece la imagen de Pokémon

  pokemonImage.src = pkmnData.sprites.other.home.front_default;

  // actualizamos las cajas de "tipos"
  ...
  pkmnData.types.forEach((t) => {
    ...
    newType.style.backgroundColor = `rgb(${typeColors[t.type.name][0]}, ${color[1]}, ${color[2]})`;


    types.appendChild(newType);
  });

  // Actualizazmos los stats y stats bar
